  ...
  const fetchMyOffers = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          // API возвращает { offers: [...], pagination: {...} }
          const offers = Array.isArray(data.data) ? data.data : (data.data?.offers || []);
          setMyOffers(offers);
        }
      }
    } catch (err) {
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1 text-blue-600">
                        <DollarSign className="h-4 w-4" />
                        <span className="text-lg">₽{(offer.myPrice || offer.price)?.toLocaleString() || '0'}</span>
                      </div>
