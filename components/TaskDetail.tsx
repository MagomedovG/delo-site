...
export function TaskDetail({ taskId, currentUserId, onBack }: TaskDetailProps) {
  ...
  // Проверить, есть ли уже отклик на эту задачу
  useEffect(() => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.data?.offers) {
            const existingOffer = data.data.offers.find(
              (o: { task_id: string }) => o.task_id === taskId
            );
            setHasOffer(!!existingOffer);
          }
        }
      } catch (err) {
  ...
  }, [taskId, currentUserId, offerSuccess]);
  ...
  const handleStartChat = async () => {
    ...
    try {
      ...
      const response = await authFetch(`/chat/tasks/${taskId}/conversations`, {
        method: "POST",
        body: JSON.stringify({
      ...
      alert("Ошибка при создании переписки");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 space-y-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xl text-blue-600">
                    ₽{task.budgetMin?.toLocaleString() || '0'} - ₽{task.budgetMax?.toLocaleString() || '0'}
                  </p>
        ...
        {/* Action Buttons for non-authors */}
        {!isAuthor && task.status === "open" && (
          <div className="sticky bottom-6 z-10 space-y-3">
            {/* Кнопка написать заказчику - показывается только если есть отклик */}
            {hasOffer && (
              <Button
                size="lg"
                variant="outline"
                className="w-full h-14 border-blue-600 text-blue-600 hover:bg-blue-50 shadow-lg"
            ...
            {/* Кнопка откликнуться - показывается только если нет отклика */}
            {!hasOffer && (
